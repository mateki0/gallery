    ...
    return(
        <AuthorWrapper>
            <AvatarImgWrapper>
                ...
                <NameWrapper>
                    <AuthorName href={`http://${profileLink.slice(12).replace("users/", "@")}?utm_source=myown-gallery&utm_medium=referral`} target="_blank">{name}</AuthorName>
                    <InstaLink insta={insta} href={`https://instagram.com/${profileLink.slice(31)}`} target="_blank">@{insta}</InstaLink>
                </NameWrapper>
            </AvatarImgWrapper>
        </AuthorWrapper>
    )
